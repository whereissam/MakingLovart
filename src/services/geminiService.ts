...
function getApiKey(capability: 'text' | 'image' | 'video' = 'text', explicitKey?: string): string {
    ...
    const key =
        scopedKey || runtimeConfig.textApiKey || runtimeConfig.imageApiKey || runtimeConfig.videoApiKey || API_KEY;
    if (!key) {
        throw new Error(
...
export async function enhancePromptWithGemini(
    ...
    try {
        ...
        if (error instanceof Error) {
            ...
            throw new Error(`Prompt enhancement failed: ${msg}`);
        }
...
export async function editImage(
    ...
    try {
        ...
        if (error instanceof Error) {
            ...
            const msg = error.message;
            if (msg.includes('API_KEY_INVALID') || msg.includes('API key not valid')) {
                throw new Error('Invalid API Key. Please verify your Google API Key in settings.');
            }
            if (msg.includes('PERMISSION_DENIED') || msg.includes('403')) {
                throw new Error('Insufficient API Key permissions. Ensure the Generative Language API is enabled.');
            }
...
export async function generateImageFromText(
    ...
    try {
        ...
        throw new Error('Unknown error generating image with Gemini API.');
    }
}
