import type { Element, ImageElement, PathElement, VideoElement } from '../types';
import { getElementBounds } from './geometry';


export const rasterizeElement = (
    element: Exclude<Element, ImageElement | VideoElement>,
        ...
        switch (element.type) {
            ...
            case 'shape': {
                ...
                if (element.shapeType === 'rectangle')
                    elementSvgString = `<rect width="${element.width}" height="${element.height}" rx="${element.borderRadius || 0}" ry="${element.borderRadius || 0}" ${shapeProps} />`;
                else if (element.shapeType === 'circle')
                    elementSvgString = `<ellipse cx="${element.width / 2}" cy="${element.height / 2}" rx="${element.width / 2}" ry="${element.height / 2}" ${shapeProps} />`;
                else if (element.shapeType === 'triangle')
                    elementSvgString = `<polygon points="${element.width / 2},0 0,${element.height} ${element.width},${element.height}" ${shapeProps} />`;
                break;
            }
            case 'arrow': {
            ...
            case 'text': {
                ...
                elementSvgString = elementSvgString
                    .replace(`x="${offsetX}"`, `x="${element.x + offsetX}"`)
                    .replace(`y="${offsetY}"`, `y="${element.y + offsetY}"`);
                break;
            }
            case 'group': {
                elementSvgString = '';
...
export const rasterizeElements = (
    elementsToRasterize: Exclude<Element, ImageElement | VideoElement>[],
): Promise<{ href: string; mimeType: 'image/png'; width: number; height: number }> => {
    return new Promise((resolve, reject) => {
        ...
        const elementSvgStrings = elementsToRasterize
            .map((element) => {
                ...
                switch (element.type) {
                    case 'path': {
                        const pointsWithOffset = element.points.map((p) => ({ x: p.x + offsetX, y: p.y + offsetY }));
                        const pathData = pointsWithOffset
