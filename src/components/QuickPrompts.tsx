...
export const QuickPrompts: React.FC<QuickPromptsProps> = ({ t, setPrompt, disabled, userEffects, onDeleteUserEffect }) => {
    ...
    return (
        <div className="relative" ref={wrapperRef}>
            ...
            {isMenuOpen && (
                <div className="absolute bottom-full left-0 mb-3 w-80 max-h-96 overflow-y-auto bg-white border border-neutral-200 rounded-xl shadow-xl p-2 flex flex-col gap-1">
                    <h4 className="px-2 pt-1 pb-2 text-xs font-semibold text-neutral-500 uppercase tracking-wider">{t('myEffects.title')}</h4>
                    {userEffects.length > 0 ? (
                        userEffects.map((effect) => (
                            <div key={effect.id} className="group flex items-center justify-between p-2 rounded-md hover:bg-neutral-100 text-neutral-800 text-sm transition-colors">
                                ...
                                <button
                                    ...
                                    className="ml-2 p-1 rounded-full opacity-0 group-hover:opacity-100 hover:bg-red-100 text-red-600 transition-opacity flex-shrink-0"
                                >
                        ...
                        <p className="px-2 pb-2 text-xs text-neutral-500">{t('myEffects.noEffects')}</p>
                    )}
                    ...
                    {builtInPrompts.map((item: {name: string, value: string}, index: number) => (
                        <button 
                            ...
                            className="block w-full text-left p-2 rounded-md hover:bg-neutral-100 text-neutral-800 text-sm transition-colors"
                        >
                            {item.name}
                        </button>
                    ))}
