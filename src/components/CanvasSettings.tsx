...
export const CanvasSettings: React.FC<CanvasSettingsProps> = ({
    ...
    return (
        <div
            className="theme-aware fixed inset-0 z-[100] flex items-center justify-center bg-black/35 backdrop-blur-sm"
            onClick={onClose}
        >
            <div
            ...
            >
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        <h3 className={`text-xl font-semibold ${isDark ? 'text-[#F3F4F6]' : 'text-[#101828]'}`}>
                            设置
                        </h3>
                ...
                <div className="space-y-6">
                    <section className="space-y-3">
                        <div
                            className={`text-xs font-semibold uppercase tracking-[0.18em] ${isDark ? 'text-[#667085]' : 'text-[#98A2B3]'}`}
                        >
                            界面主题
                        </div>
                            ...
                            {(
                                ...
                                >
                                    <div className="mb-3 flex items-center justify-between">
                                        <div
                                            className={`text-sm font-semibold ${isDark ? 'text-[#F3F4F6]' : 'text-[#101828]'}`}
                                        >
                                            {title}
                                        </div>
                                        {themeMode === mode && (
                                            <span
                                                className={`rounded-full px-2 py-1 text-[11px] font-medium ${
                                                    isDark
                                                        ? 'bg-[#7CB4FF]/20 text-[#7CB4FF]'
                                                        : 'bg-[#DCEBFF] text-[#175CD3]'
                                                }`}
                                            >
                                                当前
                                            </span>
                                    ...
                                    <div className={`mb-4 text-xs ${isDark ? 'text-[#98A2B3]' : 'text-[#667085]'}`}>
                                        {description}
                                    </div>
                                    <div
                                        className={`grid h-16 grid-cols-[1fr_56px] gap-2 rounded-2xl p-2 ${
                    ...
                    <section className="space-y-3">
                        ...
                        <div className="grid gap-3 md:grid-cols-2">
                            <div className={`rounded-2xl p-3 ${isDark ? 'bg-[#161A22]' : 'bg-[#F8FAFC]'}`}>
                                <div
                                    className={`mb-2 text-sm font-medium ${isDark ? 'text-[#D0D5DD]' : 'text-[#344054]'}`}
                                >
                                    语言
                                </div>
                                <div
                                    className={`inline-flex w-full rounded-full border p-1 ${isDark ? 'border-[#2A3140] bg-[#12151B]' : 'border-[#E4E7EC] bg-white'}`}
                                >
                                    {(
                                        [
                                            ['en', 'English'],
                                            ['zho', '中文'],
                                        ] as Array<['en' | 'zho', string]>
                                    ).map(([value, label]) => (
                                        <button
                                            key={value}
                                            type="button"
                    ...
                    <section className="space-y-3">
                        ...
                        <div
                            className={`rounded-[24px] border p-4 ${isDark ? 'border-[#2A3140] bg-[#161A22]' : 'border-[#E4E7EC] bg-[#F8FAFC]'}`}
                        >
                            <div className="grid gap-3 md:grid-cols-2">
                                <select
                                    value={provider}
                                ...
                                {validationResult && (
                                    <div
                                        className={`mt-2 rounded-xl px-3 py-2 text-sm ${
                                            validationResult.ok
                                                ? isDark
                                                    ? 'bg-[#123524] text-[#75E0A7]'
                                                    : 'bg-[#ECFDF3] text-[#027A48]'
                                                : isDark
                                                  ? 'bg-[#3A1616] text-[#FDA29B]'
                            ...
                            {userApiKeys.length === 0 ? (
                                ...
                                >
                                    ...
                                    <div className="mt-1 text-xs">
                                        在上方选择供应商、粘贴 Key，点击「验证并保存」即可开始使用 AI 能力
                                    </div>
                                </div>
                            ) : (
                                    ...
                                    >
                                        <div className="min-w-0">
                                            <div className="flex items-center gap-2">
                                                ...
                                                />
                                                <span
                                                    className={`truncate text-sm font-medium ${isDark ? 'text-[#F3F4F6]' : 'text-[#101828]'}`}
                                                >
                                                    {item.name || item.provider}
                                                </span>
                                            </div>
                                            <div
                                                className={`mt-1 text-xs ${isDark ? 'text-[#98A2B3]' : 'text-[#667085]'}`}
                                            >
                                                {maskKey(item.key)}
                                            </div>
                                        ...
                                        <div className="ml-3 flex items-center gap-2">
                                            {!item.isDefault ? (
                                                <button
                                                    type="button"
                                                    onClick={() => onSetDefaultApiKey(item.id)}
                                                    className={chipClass}
                                                >
                                                    设为默认
                                                </button>
                    ...
                    <section className="space-y-3">
                        ...
                        <div className="grid gap-3 md:grid-cols-2">
                            <label className={`rounded-2xl p-3 ${isDark ? 'bg-[#161A22]' : 'bg-[#F8FAFC]'}`}>
                                ...
                                <select
                                    value={modelPreference.textModel}
                                    onChange={(event) =>
                                        setModelPreference({ ...modelPreference, textModel: event.target.value })
                                    }
                                    className={inputClass}
                                >
                            ...
                            <label className={`rounded-2xl p-3 ${isDark ? 'bg-[#161A22]' : 'bg-[#F8FAFC]'}`}>
                                ...
                                <select
                                    value={modelPreference.imageModel}
                                    onChange={(event) =>
                                        setModelPreference({ ...modelPreference, imageModel: event.target.value })
                                    }
                                    className={inputClass}
                                >
                            ...
                            <label className={`rounded-2xl p-3 ${isDark ? 'bg-[#161A22]' : 'bg-[#F8FAFC]'}`}>
                                ...
                                <select
                                    value={modelPreference.videoModel}
                                    onChange={(event) =>
                                        setModelPreference({ ...modelPreference, videoModel: event.target.value })
                                    }
                                    className={inputClass}
                                >
                                    {modelOptions.video.map((model) => (
                                        <option key={model} value={model}>
                                            {model}
                                        </option>
                                    ))}
                            ...
                            <label className={`rounded-2xl p-3 ${isDark ? 'bg-[#161A22]' : 'bg-[#F8FAFC]'}`}>
                                ...
                                <select
                                    value={modelPreference.agentModel}
                                    onChange={(event) =>
                                        setModelPreference({ ...modelPreference, agentModel: event.target.value })
                                    }
                                    className={inputClass}
                                >
                                    {modelOptions.agent.map((model) => (
                                        <option key={model} value={model}>
                                            {model}
                                        </option>
                                    ))}
            ...
            {showConfigForm && (
                <ConfigForm
                    ...
                    onSave={(draft) => {
                        apiConfigStore.addConfig(draft);
                    }}
                    onUpdate={(id, patch) => {
                        apiConfigStore.updateConfig(id, patch);
                    }}
                    onClose={() => {
